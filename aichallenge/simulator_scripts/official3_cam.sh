#!/bin/bash
# The official three-car race with the CAMERA ON, for the camera-based AI.
#
# Every scenario here runs --camera off, so pilot_net -- a PilotNet/DAVE-2 taking /sensing/camera/image_raw --
# has never had an input. That is why this approach was never tested, independently of whether its weights are
# any good.
#
# It also matters for the AI division's sensor list, which permits camera, lidar, steering and wheel odometry.
# Camera on costs CPU, so it is a separate script rather than a change to the lidar scenarios.
